...
myApp.controller('facultyCtrl', ['$scope', '$http', '$window', '$log', '$location', function($scope, $http, $window, $log, $location) {
    ...
    $scope.goToCourse = function(email, courseName) {
        ...
        $window.location.href=("/viewcourse_faculty.html" + "?email=" + email + "?course=" + courseName);
    }

    $scope.addNewCourse = function(email) {
        $window.location.href=("/add_course.html" + "?email=" + email);
    }

    $scope.goToProfile = function() {
        $window.location.href=("/profile.html" + "?email=" + $scope.facultyEmail);
    }
}])
